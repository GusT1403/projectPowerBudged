...
function OltForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    if (!powerValid || !connectorValid || !couplerValid || !fusionValid || !maxDistanceValid) {
      ...
      <div className="wrapper2" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={onSubmit}>
          ...
          <div className="input-box">
            ...
            <input id="power" type="text" placeholder="decimal number separated by a dot" {...register('power')} autoFocus />
            {errors.power && <span className="error">{errors.power.message}</span>}
          </div>
          <div className="input-box">
            <label htmlFor="connector">Connector loss [db]</label>
            <input id="connector" type="text" placeholder="decimal number separated by a dot" {...register('connector')} />
            {errors.connector && <span className="error">{errors.connector.message}</span>}
          </div>
          <div className="input-box">
            <label htmlFor="coupler">Coupler duplex adapter loss [db]</label>
            <input id="coupler" type="text" placeholder="decimal number separated by a dot" {...register('coupler')} />
            {errors.coupler && <span className="error">{errors.coupler.message}</span>}
          </div>
          <div className="input-box">
            <label htmlFor="fusion">Fusion or splice loss [db]</label>
            <input id="fusion" type="text" placeholder="decimal number separated by a dot" {...register('fusion')} />
            {errors.fusion && <span className="error">{errors.fusion.message}</span>}
          </div>
          <div className="input-box">
            <label htmlFor="maxDistance">Maximum link distance [km]</label>
            <input id="maxDistance" type="text" placeholder="decimal number separated by a dot" {...register('maxDistance')} />
            {errors.maxDistance && <span className="error">{errors.maxDistance.message}</span>}
          </div>
